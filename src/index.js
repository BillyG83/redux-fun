...
function listItemClicked (event) {
    ...
    if (complete === 'false') {
        store.dispatch(action.itemComplete(Number(event.target.id)))
    } else {
        store.dispatch(action.itemRemoved(Number(event.target.id)))
    }
...
function addButtonClicked() {
    ...
    input.value && store.dispatch(action.itemAdded(input.value))
    input.value = ''
    updateList()
}
